...
# Workaround Haystack issue 575
import whoosh.searching
@patch(whoosh.searching.Searcher, 'search')
def Searcher_with_workaround_for_haystack_issue_575(original_function, self, q, **kwargs):
    if 'limit' not in kwargs:
        kwargs['limit'] = None
    return original_function(self, q, **kwargs)
...
import haystack.backends.whoosh_backend
@patch(haystack.backends.whoosh_backend.WhooshSearchBackend, 'search')
def Searcher_with_support_for_sorting_by_multiple_fields(original_function,
    ...
    if sort_by is not None:
        ...
        # temporarily patch Searcher.search_page to change the sort order to
        # our replacement calculated above

        def temporary_Searcher_search_page(original_function, self, query,
            pagenum, pagelen=10, **kwargs):

            kwargs['sortedby'] = sort_by
            kwargs['reverse'] = reverse

            return original_function(self, query, pagenum, pagelen, **kwargs)

        from whoosh.searching import Searcher
        with patch(Searcher, 'search_page', temporary_Searcher_search_page):
            return original_function(self, *args, **kwargs)
    else:
        return original_function(self, *args, **kwargs)
